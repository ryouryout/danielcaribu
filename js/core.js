...
function gboot() {
  ...
  async function initializeApp() {
    ...
    await lang_init(app, handleLanguageChange, show_welcome_modal);
    show_welcome_modal();
...
async function continue_connection({data, device}) {
  try {
    ...
    if (model == "DS4" && info?.rare) {
      show_popup("この DualShock 4 は一般的でない個体情報を返しました。作業前に情報タブの内容を控えてから進めてください。");
    }
    ...
    if(model == "VR2") {
      show_popup("<p>PS VR2 Sense コントローラーは試験対応です。</p><p>保存前に、中心校正・可動域校正・表示結果を必ず確認してください。</p><p>永続保存は挙動を確認してから行ってください。</p>", true)
    }
...
function close_all_modals() {
  hideAllVisibleModals();
}

function render_info_to_dom(infoItems) {
  // Clear all info sections
  $("#fwinfo").empty();
  $("#fwinfoextra-hw").empty();
  $("#fwinfoextra-fw").empty();
  ...
  infoItems.forEach(({key, value, addInfoIcon, severity, isExtra, cat, copyable}) => {
    ...
    if (isExtra) {
      appendInfoExtra(key, value, severity, cat || "hw", copyable ?? false);
    } else {
      appendInfo(key, value, severity, cat || "hw", copyable ?? false);
    }
  });
}
...
function copyValueToClipboard(text) {
  if (!navigator.clipboard?.writeText) {
    errorAlert(l("Cannot copy text to the clipboard:") + " クリップボード機能が利用できません");
    return;
  }
...
function buildInfoRow(key, value, severity, copyable, extraLayout = false) {
  const dt = document.createElement('dt');
  dt.className = extraLayout ? 'text-muted col-sm-4 col-md-6 col-xl-5' : 'text-muted col-6';
  dt.textContent = key;

  const dd = document.createElement('dd');
  dd.className = extraLayout ? 'col-sm-8 col-md-6 col-xl-7 info-value-cell' : 'col-6 info-value-cell';
  ...
  if (copyable) {
    ...
    copyButton.className = 'btn btn-link info-copy-btn';
    copyButton.innerHTML = '<i class="fa-regular fa-copy"></i>';
    copyButton.addEventListener('click', () => copyValueToClipboard(value));
...
function appendInfoExtra(key, value, severity, cat, copyable) {
  const container = document.getElementById(`fwinfoextra-${cat}`);
  if (!container) return;

  const [dt, dd] = buildInfoRow(key, value, severity, copyable, true);
  container.appendChild(dt);
  container.appendChild(dd);
}


function appendInfo(key, value, severity, cat, copyable) {
  const container = document.getElementById('fwinfo');
  if (!container) return;

  const [dt, dd] = buildInfoRow(key, value, severity, copyable, false);
  container.appendChild(dt);
  container.appendChild(dd);
  appendInfoExtra(key, value, severity, cat, copyable);
}
