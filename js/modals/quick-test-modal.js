...
export class QuickTestModal {
  ...
  _loadSkippedTestsFromStorage() {
    try {
      ...
      if (Array.isArray(skippedTests) && skippedTests.length > 0) {
        ...
        this.state.skippedTests = skippedTests.filter(test =>
          TEST_SEQUENCE.includes(test) && supportedTests.includes(test)
        );
  ...
  _buildDynamicAccordion() {
    ...
    const activeTests = this._getAvailableTests();
  ...
  _areAllTestsCompleted() {
    return this._getAvailableTests().every(test => this.state[test] !== null);
  }
  ...
  async _startLightsTest() {
    ...
    if (!currentController?.setLightbarColor || !currentController?.setPlayerIndicator) {
      ...
      alert('このコントローラーではライトテストを利用できません。');
  ...
  _updateTestSummary() {
    ...
    const totalTests = activeTests.length + skipped;

    if (totalTests === 0) {
      $summary.text(l('No tests completed yet.'));
      $summary.attr('class', 'text-muted');
    } else {
      let summaryText = `${completed}/${totalTests} ${l("tests completed")}. ${passed} ${l("passed")}, ${completed - passed} ${l("failed")}.`;
      if (skipped > 0) {
        summaryText += ` ${skipped} ${l("skipped")}.`;
      }
      $summary.text(summaryText);
      $summary.attr('class', completed === activeTests.length ? 'text-success' : 'text-info');
    }
  }
  ...
  _expandNextTest(currentTest) {
    const availableTests = this._getAvailableTests();
    const currentIndex = availableTests.indexOf(currentTest);
    ...
    for (let i = currentIndex + 1; i < availableTests.length; i++) {
      const nextTest = availableTests[i];
  ...
  _getCurrentActiveTest() {
    for (const test of this._getAvailableTests()) {
  ...
  _isFirstTest(testType) {
    const [firstTest] = this._getAvailableTests();
  ...
  async _startTestSequence() {
    ...
    setTimeout(() => {
      const [firstAvailableTest] = this._getAvailableTests();
  ...
  _moveToPreviousTest() {
    ...
    const availableTests = this._getAvailableTests();
    const currentIndex = availableTests.indexOf(activeTest);
    ...
    const previousTest = availableTests[previousIndex];
